...
public class HomeworkToDtoConverter implements Converter<Homework, HomeworkDto> {
	...
	public HomeworkDto convert(Homework source) {
		return new HomeworkDto(source.getId(), StandardCharsets.UTF_8.decode(ByteBuffer.wrap(source.getBytes())).toString(), source.getFileName(), source.getFileExtension(), source.getUserId());
	}
}